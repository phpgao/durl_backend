...
// First returns the first TLeaf entity from the query.
// Returns a *NotFoundError when no TLeaf was found.
func (tq *TLeafQuery) First(ctx context.Context) (*TLeaf, error) {
	...
	if len(nodes) == 0 {
		return nil, &NotFoundError{tleaf.Label}
	}
	return nodes[0], nil
}
...
// FirstID returns the first TLeaf ID from the query.
// Returns a *NotFoundError when no TLeaf ID was found.
...
// Only returns a single TLeaf entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when more than one TLeaf entity is found.
// Returns a *NotFoundError when no TLeaf entities are found.
func (tq *TLeafQuery) Only(ctx context.Context) (*TLeaf, error) {
	...
	case 0:
		return nil, &NotFoundError{tleaf.Label}
	default:
		return nil, &NotSingularError{tleaf.Label}
	}
}
...
// OnlyID is like Only, but returns the only TLeaf ID in the query.
// Returns a *NotSingularError when more than one TLeaf ID is found.
...
func (tq *TLeafQuery) OnlyID(ctx context.Context) (id int64, err error) {
	...
	case 0:
		err = &NotFoundError{tleaf.Label}
	default:
		err = &NotSingularError{tleaf.Label}
	}
	return
}
...
func (tq *TLeafQuery) All(ctx context.Context) ([]*TLeaf, error) {
	...
	qr := querierAll[[]*TLeaf, *TLeafQuery]()
	return withInterceptors[[]*TLeaf](ctx, tq, qr, tq.inters)
}
...
func (tq *TLeafQuery) IDs(ctx context.Context) (ids []int64, err error) {
	...
	if err = tq.Select(tleaf.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}
...
func (tq *TLeafQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, tq, querierCount[*TLeafQuery](), tq.inters)
}
...
// Clone returns a duplicate of the TLeafQuery builder, including all associated steps. It can be
...
func (tq *TLeafQuery) Clone() *TLeafQuery {
	...
	return &TLeafQuery{
		config:     tq.config,
		ctx:        tq.ctx.Clone(),
		order:      append([]tleaf.OrderOption{}, tq.order...),
		inters:     append([]Interceptor{}, tq.inters...),
		predicates: append([]predicate.TLeaf{}, tq.predicates...),
...
func (tq *TLeafQuery) GroupBy(field string, fields ...string) *TLeafGroupBy {
	...
	grbuild := &TLeafGroupBy{build: tq}
	grbuild.flds = &tq.ctx.Fields
	grbuild.label = tleaf.Label
...
func (tq *TLeafQuery) Select(fields ...string) *TLeafSelect {
	...
	sbuild := &TLeafSelect{TLeafQuery: tq}
	sbuild.label = tleaf.Label
...
func (tq *TLeafQuery) prepareQuery(ctx context.Context) error {
...
func (tq *TLeafQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*TLeaf, error) {
	var (
		nodes = []*TLeaf{}
		_spec = tq.querySpec()
	)
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*TLeaf).scanValues(nil, columns)
	}
	_spec.Assign = func(columns []string, values []any) error {
		node := &TLeaf{config: tq.config}
...
func (tq *TLeafQuery) sqlCount(ctx context.Context) (int, error) {
	_spec := tq.querySpec()
	if len(tq.modifiers) > 0 {
		_spec.Modifiers = tq.modifiers
	}
...
func (tq *TLeafQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(tleaf.Table, tleaf.Columns, sqlgraph.NewFieldSpec(tleaf.FieldID, field.TypeInt64))
	_spec.From = tq.sql
...
func (tq *TLeafQuery) sqlQuery(ctx context.Context) *sql.Selector {
	builder := sql.Dialect(tq.driver.Dialect())
	t1 := builder.Table(tleaf.Table)
	columns := tq.ctx.Fields
	if len(columns) == 0 {
		columns = tleaf.Columns
	}
...
func (tgb *TLeafGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*TLeafQuery, *TLeafGroupBy](ctx, tgb.build, tgb, tgb.build.inters, v)
}

func (tgb *TLeafGroupBy) sqlScan(ctx context.Context, root *TLeafQuery, v any) error {
...
func (ts *TLeafSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*TLeafQuery, *TLeafSelect](ctx, ts.TLeafQuery, ts, ts.inters, v)
}

func (ts *TLeafSelect) sqlScan(ctx context.Context, root *TLeafQuery, v any) error {
